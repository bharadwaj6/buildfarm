...
public class InMemoryLRUCASAdapter implements CASAdapter {
  ...
  private FlushResult flushAllEntries() {
    try {
      ...
      return new FlushResult(false, "Failed to flush in-memory LRU CAS entries: " + e.getMessage(), 0, 0);
    }
  }
  ...
  private FlushResult flushInstanceEntries(String instanceName) {
    ...
    String message = String.format(
        "Instance-specific flush not supported for in-memory LRU CAS. No entries removed for instance %s",
        instanceName);
  ...
  private FlushResult flushDigestPrefixEntries(String digestPrefix) {
    try {
      ...
      String message = String.format(
          "Digest prefix-specific flush not supported for in-memory LRU CAS. No entries removed for prefix %s",
          digestPrefix);
      
      return new FlushResult(true, message, 0, 0);
    } catch (Exception e) {
      logger.log(Level.SEVERE, "Failed to flush in-memory LRU CAS entries with digest prefix: " + digestPrefix, e);
      return new FlushResult(false, "Failed to flush in-memory LRU CAS entries: " + e.getMessage(), 0, 0);
    }
  }
