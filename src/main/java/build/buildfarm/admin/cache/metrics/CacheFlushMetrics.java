...
public class CacheFlushMetrics {
  private static final Counter flushOperationsCounter = Counter.build()
      .name("cache_flush_operations_total")
      .labelNames("cache_type", "backend", "scope", "success")
      .help("Total number of cache flush operations.")
      .register();
  
  private static final Counter entriesRemovedCounter = Counter.build()
  ...
  private static final Counter bytesReclaimedCounter = Counter.build()
  ...
  public static void recordFlushOperation(
    ...
    flushOperationsCounter.labels(
        cacheType, backend, scope, String.valueOf(success)).inc();
  }
   ...
   */
  public static void recordEntriesRemoved(
      String cacheType, String backend, int entriesRemoved) {
    if (entriesRemoved > 0) {
   ...
   */
  public static void recordBytesReclaimed(
      String cacheType, String backend, long bytesReclaimed) {
    if (bytesReclaimed > 0) {
  ...
  /**
   * Gets the total number of flush operations for a specific cache type, backend, scope, and success status.
   *
   * @param cacheType the type of cache (action-cache or cas)
   * @param backend the backend (redis, in-memory, filesystem, etc.)
   * @param scope the scope of the flush operation (ALL, INSTANCE, DIGEST_PREFIX)
  ...
  public static Map<String, Object> getMetricsSummary() {
    ...
    actionCacheMetrics.put("operations_success", getFlushOperationsCount("action-cache", "redis", "ALL", true) + 
                                               getFlushOperationsCount("action-cache", "in-memory", "ALL", true));
    actionCacheMetrics.put("operations_failure", getFlushOperationsCount("action-cache", "redis", "ALL", false) + 
                                               getFlushOperationsCount("action-cache", "in-memory", "ALL", false));
    actionCacheMetrics.put("entries_removed", getEntriesRemovedCount("action-cache", "redis") + 
    ...
    casMetrics.put("operations_success", getFlushOperationsCount("cas", "filesystem", "ALL", true) + 
                                       getFlushOperationsCount("cas", "in-memory-lru", "ALL", true) + 
                                       getFlushOperationsCount("cas", "redis-worker-map", "ALL", true));
    casMetrics.put("operations_failure", getFlushOperationsCount("cas", "filesystem", "ALL", false) + 
                                       getFlushOperationsCount("cas", "in-memory-lru", "ALL", false) + 
                                       getFlushOperationsCount("cas", "redis-worker-map", "ALL", false));
    casMetrics.put("entries_removed", getEntriesRemovedCount("cas", "filesystem") + 
                                    getEntriesRemovedCount("cas", "in-memory-lru") + 
                                    getEntriesRemovedCount("cas", "redis-worker-map"));
    casMetrics.put("bytes_reclaimed", getBytesReclaimedCount("cas", "filesystem") + 
