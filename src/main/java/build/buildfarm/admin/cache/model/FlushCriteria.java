...
public class FlushCriteria {
  ...
  public FlushCriteria(FlushScope scope, String instanceName, String digestPrefix) {
    ...
    if (scope == FlushScope.INSTANCE) {
      Preconditions.checkNotNull(instanceName, "instanceName must be provided when scope is INSTANCE");
    }
    
    if (scope == FlushScope.DIGEST_PREFIX) {
      Preconditions.checkNotNull(digestPrefix, "digestPrefix must be provided when scope is DIGEST_PREFIX");
    }
