...
/**
 * Implementation of the {@link CacheFlushService} interface. This service coordinates flush
...
public class CacheFlushServiceImpl implements CacheFlushService {
  ...
  public CacheFlushServiceImpl(
      Map<String, ActionCacheAdapter> actionCacheAdapters, Map<String, CASAdapter> casAdapters) {
    this(
        actionCacheAdapters,
        casAdapters,
        new ConcurrencyControlService(ConcurrencyConfig.getDefault()));
  }
  ...
  public CacheFlushServiceImpl(
    ...
    this.actionCacheAdapters =
        ImmutableMap.copyOf(Preconditions.checkNotNull(actionCacheAdapters, "actionCacheAdapters"));
    this.casAdapters = ImmutableMap.copyOf(Preconditions.checkNotNull(casAdapters, "casAdapters"));
    this.concurrencyControlService =
        Preconditions.checkNotNull(concurrencyControlService, "concurrencyControlService");
  }
  ...
  public ActionCacheFlushResponse flushActionCache(ActionCacheFlushRequest request) {
    ...
    FlushCriteria criteria =
        new FlushCriteria(request.getScope(), request.getInstanceName(), request.getDigestPrefix());
  ...
  private void validateActionCacheFlushRequest(ActionCacheFlushRequest request) {
    ...
    if (request.getScope() == build.buildfarm.admin.cache.model.FlushScope.INSTANCE
      ...
      throw new IllegalArgumentException("Instance name must be specified when scope is INSTANCE");
    }
    ...
    if (!request.isFlushRedis() && !request.isFlushInMemory()) {
      throw new IllegalArgumentException("At least one backend must be selected for flushing");
    }
  }
  ...
  private FlushResult flushActionCacheBackend(String backendName, FlushCriteria criteria) {
    ...
    try {
      ...
      return new FlushResult(
          false,
          "Error flushing Action Cache backend: " + backendName + ": " + e.getMessage(),
          0,
          0);
    }
  }
  ...
  private FlushResult flushCASBackend(String backendName, FlushCriteria criteria) {
    ...
    try {
      ...
      CacheFlushMetrics.recordEntriesRemoved("cas", backendName, result.getEntriesRemoved());
      CacheFlushMetrics.recordBytesReclaimed("cas", backendName, result.getBytesReclaimed());

      return result;
    } catch (Exception e) {
      logger.log(Level.SEVERE, "Error flushing CAS backend: " + backendName, e);
      // Record failed operation in metrics
      CacheFlushMetrics.recordFlushOperation("cas", backendName, criteria.getScope().name(), false);
      return new FlushResult(
