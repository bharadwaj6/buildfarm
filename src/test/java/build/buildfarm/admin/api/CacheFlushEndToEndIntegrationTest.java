...
/**
 * End-to-end integration tests for the Cache Flush API. These tests use real implementations of the
 * adapters with in-memory storage.
 */
...
public class CacheFlushEndToEndIntegrationTest {
  ...
  public void setUp() {
    ...
    ConcurrencyControlService concurrencyControlService =
        new ConcurrencyControlService(concurrencyConfig);
  ...
  private void populateTestData() {
    ...
    for (int i = 0; i < 50; i++) {
      ...
      inMemoryActionCacheAdapter.put(
          key, instanceName, digestPrefix + UUID.randomUUID().toString(), "test-data");
    }
    ...
    for (int i = 0; i < 50; i++) {
      ...
      inMemoryLRUCASAdapter.put(
          key, instanceName, digestPrefix + UUID.randomUUID().toString(), data);
    }
  }
  ...
  public void testFlushDigestPrefixSpecificActionCache() {
    ...
    assertTrue(
        Math.abs(expectedRemoved - actualRemoved)
            <= 2); // Allow for small variation due to random distribution
  ...
  public void testFlushAllCAS() {
    ...
    assertEquals(1, flushResponse.getEntriesRemovedByBackend().size());
    assertEquals(
        Integer.valueOf(50), flushResponse.getEntriesRemovedByBackend().get("in-memory-lru"));
  ...
  public void testFlushInstanceSpecificCAS() {
    ...
    assertEquals(1, flushResponse.getEntriesRemovedByBackend().size());
    assertEquals(
        Integer.valueOf(25), flushResponse.getEntriesRemovedByBackend().get("in-memory-lru"));
  ...
  private static class InMemoryLRUCASAdapter implements CASAdapter {
    ...
    public FlushResult flushEntries(FlushCriteria criteria) {
      ...
      return new FlushResult(
          true,
          "Flushed " + entriesRemoved + " entries (" + bytesReclaimed + " bytes)",
          entriesRemoved,
          bytesReclaimed);
    }
  }
