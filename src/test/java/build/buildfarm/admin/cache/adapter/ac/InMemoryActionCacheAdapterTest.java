...
public class InMemoryActionCacheAdapterTest {
  ...
  public void setUp() {
    ...
    cache.put(
        new ActionCacheKey("instance1", createDigest("abc123")), ActionResult.getDefaultInstance());
    cache.put(
        new ActionCacheKey("instance1", createDigest("def456")), ActionResult.getDefaultInstance());
    cache.put(
        new ActionCacheKey("instance2", createDigest("abc789")), ActionResult.getDefaultInstance());
  }
  ...
  private Digest createDigest(String hash) {
    return Digest.newBuilder().setHash(hash).setSizeBytes(123).build();
  }
}
