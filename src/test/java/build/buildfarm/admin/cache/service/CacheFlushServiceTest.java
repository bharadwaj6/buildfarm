    ...
    service = new CacheFlushService() {
      ...
      public CASFlushResponse flushCAS(CASFlushRequest request) {
        ...
        if (request.isFlushFilesystem()) {
          ...
          response.addBackendResult("filesystem", result.getEntriesRemoved(), result.getBytesReclaimed());
          if (!result.isSuccess()) {
            response.setSuccess(false);
            response.setMessage(response.getMessage() + (response.getMessage().isEmpty() ? "" : ", ") 
                + result.getMessage());
        ...
        if (request.isFlushInMemoryLRU()) {
          ...
          response.addBackendResult("in-memory-lru", result.getEntriesRemoved(), result.getBytesReclaimed());
          if (!result.isSuccess()) {
            response.setSuccess(false);
            response.setMessage(response.getMessage() + (response.getMessage().isEmpty() ? "" : ", ") 
                + result.getMessage());
        ...
        if (request.isFlushRedisWorkerMap()) {
          ...
          response.addBackendResult("redis-worker-map", result.getEntriesRemoved(), result.getBytesReclaimed());
          if (!result.isSuccess()) {
            response.setSuccess(false);
            response.setMessage(response.getMessage() + (response.getMessage().isEmpty() ? "" : ", ") 
                + result.getMessage());
